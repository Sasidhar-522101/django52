...
def FlightView(request):
    form = FlightForm(request.POST)
    flights = None  # Initialize flights variable to None
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            # Query Flights model based on selected source, destination, and date
            flights = Flights.objects.filter(source=source, destination=destination, date=date)
...
def search_flights(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Query Flights model based on selected cities and date
            flights = Flights.objects.filter(source=source, destination=destination, date=date)

            # Pass flight information to the template
            return render(request, 'flight_search_results.html', {'flights': flights, 'source': source, 'destination': destination, 'date': date})
    else:
        form = FlightForm()
